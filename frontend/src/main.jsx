...
const money = (value = 0) => new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 }).format(Number(value) || 0);
...
function App() {
  ...
  const refresh = React.useCallback(async () => {
    try {
      ...
      return true;
    } catch { setLive(false); return false; }
  }, []);
  ...
  const execute = async (item) => {
    ...
    try {
      ...
      if (result.status === 'ESCALATED') {
        ...
        setNotice({ type: 'success', text: `${pretty(result.action)} completed — ${money(result.amount_recovered)} recovered.` });
      }
      await refresh();
    } catch (e) { setNotice({ type: 'error', text: e.message }); }
    finally { setLoading(false); }
  ...
  const reset = async () => { const result = await action(api.resetDemo, 'Demo database reset complete.'); if (result) { setFailureArmed(false); setBatchResult(null); } };
  const batch = async () => {
    const result = await action(api.runRecoveryBatch, 'Batch recovery completed. Metrics refreshed.');
