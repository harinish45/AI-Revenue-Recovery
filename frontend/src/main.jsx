...
function StatusPill({ children, type = '' }) {
  return <span className={`pill ${String(type).toLowerCase().replaceAll('_', '-')}`}>{children}</span>;
}

function MetricCard({ label, value, note, tone = '' }) {
  return <section className={`metric-card ${tone}`}><div className="metric-label">{label}</div><div className="metric-value">{value}</div><div className="metric-note">{note}</div></section>;
}
...
function App() {
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      ...
      setNotice({ type: 'warning', text: `Backend not connected — showing demo data. ${error.message}` });
  ...
  const runAction = async (label, action) => {
    ...
    try {
      ...
      setNotice({ type: result?.status === 'FAILED' ? 'danger' : 'success', text: result?.message || `${label} completed.` });
      await refresh();
    } catch (error) {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <div className="brand-block"><div className="brand-mark">R</div><div><div className="brand-name">RecoverAI</div><div className="brand-subtitle">Revenue recovery</div></div></div>
      <nav className="nav-list">{nav.map((item) => <button key={item} className={`nav-item ${active === item ? 'active' : ''}`} onClick={() => setActive(item)}><span className="nav-dot" />{item}</button>)}</nav>
      <div className="sidebar-footer"><div className="test-mode"><span className="mode-dot" />Razorpay Test Mode</div><div className="version">Pitch build · v0.2</div></div>
    </aside>

    <main className="main-content">
      <header className="topbar">
        <div><div className="eyebrow">Merchant control center</div><h1>{active === 'Overview' ? 'Revenue recovery overview' : active}</h1></div>
        <div className="topbar-actions"><button className="ghost-btn" disabled={loading} onClick={() => runAction('Demo reset', api.resetDemo)}>Reset demo</button><button className="primary-btn" disabled={loading} onClick={() => runAction('Recovery batch', api.runRecoveryBatch)}>Run recovery batch</button></div>
      </header>
      ...
      {active === 'Overview' && <>
        <section className="hero-banner"><div><div className="hero-kicker">AI revenue recovery</div><h2>Turn failed payments back into cash.</h2><p>RecoverAI detects revenue at risk, selects a bounded intervention, executes safely, and explains every money action.</p></div><div className="hero-status"><span className="status-ring" />Agent operational</div></section>
        <section className="metric-grid">{metrics.map((metric) => <MetricCard key={metric.label} {...metric} />)}</section>
        <section className="content-grid">
          <div className="panel large-panel"><div className="panel-header"><div><h3>Recovery queue</h3><p>Prioritized cases requiring an action.</p></div><button className="text-btn" onClick={() => setActive('Recovery queue')}>View all</button></div>
            <div className="queue-toolbar"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search customer, case or failure..." /><span>{filteredCases.length} cases</span></div>
            <div className="table-wrap"><table><thead><tr><th>Customer</th><th>Amount</th><th>Failure</th><th>Risk</th><th>AI action</th><th>Status</th></tr></thead><tbody>{filteredCases.map((item) => <tr key={item.id} onClick={() => openCase(item)} className="clickable-row"><td><div className="customer-cell"><div className="avatar">{(item.customer?.name || 'NA').split(' ').map((x) => x[0]).join('').slice(0, 2)}</div><div><strong>{item.customer?.name || 'Unknown'}</strong><span>{item.id}</span></div></div></td><td className="amount">{money(item.amount)}</td><td>{item.failure_reason}</td><td><StatusPill type={item.risk_level}>{item.risk_level}</StatusPill></td><td>{prettyAction(item.recommended_action)}</td><td><StatusPill type={item.status}>{prettyAction(item.status)}</StatusPill></td></tr>)}</tbody></table></div>
          </div>
          <div className="panel agent-panel"><div className="panel-header"><div><h3>Latest agent decision</h3><p>RC-001 · ₹2,499 at risk</p></div><StatusPill type="HIGH">HIGH</StatusPill></div><div className="decision-card"><div className="decision-label">Decision</div><div className="decision-title">Retry payment</div><p>Customer history and policy checks indicate this payment is eligible for one bounded recovery attempt.</p><div className="checks"><div><span className="check">✓</span>Test mode only</div><div><span className="check">✓</span>Retry count 0 / 2</div><div><span className="check">✓</span>Amount below policy limit</div></div></div><button className="recover-btn" onClick={() => openCase(cases[0])}>Review recovery action</button></div>
        </section>
        <section className="bottom-grid"><div className="panel"><div className="panel-header"><div><h3>Recovery performance</h3><p>Current batch · {summary.recovery_attempts || 0} attempts</p></div><div className="trend">{summary.recovery_rate || 0}%</div></div><div className="bar-chart">{[42,60,52,72,66,84,78,92].map((height, index) => <div className="bar-group" key={index}><div className="bar" style={{ height: `${height}%` }} /><span>W{index + 1}</span></div>)}</div></div><AuditPanel audit={audit} onOpen={() => setActive('Audit trail')} /></section>
      </>}
