...
def test_voice_follow_up_is_saved_for_human_review_case(client: TestClient):
    ...
    voice_events = (
        ("voice_call_started", "DISCOVERY"),
        ("voice_call_ended", "MANUAL_END"),
    )
    for event_type, intent in voice_events:
        response = client.post(
            f"/api/cases/{case_id}/voice-events",
    ...
    items = client.get("/api/cases", params={"limit": 100}).json()["items"]
    case = next(item for item in items if item["id"] == case_id)
